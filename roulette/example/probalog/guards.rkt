...
;;
;; A guard says under what conditions an element is present in a set,
;; and is a BDD. Canonicality is what the engine relies on: redundant
;; disjuncts collapse on contact, equivalence is a pointer comparison,
;; and a marginal is a weighted count over a diagram that already
;; exists. Building the diagram as the derivation proceeds rather than
;; compiling at the end is TP-compilation (Vlasselaer et al., IJAR 2016).
...
;; The manager
;;
;; One per process. `reset-guards!` invalidates every existing guard:
;; they name variables in a manager that is gone, and nothing can detect
;; that afterwards -- an operation on one may fault or may quietly
;; return a wrong answer. Callers holding guards across a reset must
;; drop them, which is what the hooks below are for.
...
;; probalog keeps no variable symbolic, so the kept-map is empty
